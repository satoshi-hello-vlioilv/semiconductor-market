...
const COMPANIES=[
  ...
  {id:'oxide',layer:1,sub:'特殊材料',name:'オキサイド',en:'Oxide Inc.',ticker:'6521.T',country:'JP',
   ...
   featured:true,gnt:true,revenue:'≈¥30億',mcap:'≈¥150億規模',stock24:'年初来高値圏（2026）',
  ...
  {id:'arm',layer:3,sub:'CPU IPコア',name:'ARM Holdings',en:'Arm Holdings plc',ticker:'ARM',country:'UK',
   ...
   featured:true,
   revenue:'$4.92B（FY2026・+23%）',mcap:'≈$400B規模',stock24:'+約90%（直近1年）',perf:'up',
  ...
  {id:'skhynix',layer:5,sub:'メモリIDM',name:'SK hynix',ticker:'000660.KS',country:'KR',
   ...
   featured:true,
   revenue:'$44.2B（2024）→過去最高（2025）',mcap:'≈$1.1兆ドル（2026年突破）',stock24:'+約250%（2026年初来）',perf:'up',
  ...
  {id:'kioxia',layer:5,sub:'メモリIDM',name:'キオクシア（Kioxia）',en:'Kioxia Holdings',ticker:'285A.T',country:'JP',
   ...
   featured:true,
   revenue:'≈¥3兆（FY2025・過去最高）',mcap:'≈¥44兆（日本最大級）',stock24:'年初来+約660%（上場来 約56倍）',perf:'up',
...
/* ─── Build Stock Ranking ─── */
const STOCK_RANKING=[
  {rank:1,id:'oxide',name:'オキサイド',ticker:'6521.T',country:'JP',currRef:'¥6,500前後',tgtRef:'¥22,500',multi:3.5,horizon:'18ヶ月',moat:'DUV波長変換単結晶95%独占',catalysts:['AI検査装置需要の急増','EUV光源コンポーネント展開','KLA・日立ハイテク採用継続'],rationale:'時価総額≈¥150億という極めて小さな規模に対し、代替品ゼロの95%独占という構造的優位性が非対称リターンを生む。売上¥30億規模のため微細化進展に伴う受注増が利益に直結。AI GPU量産増で検査装置需要は構造的拡大局面にあり、小型株特有の営業レバレッジが最大の魅力。',risk:'流動性が低く機関投資家の参入が制限される。半導体サイクル感応が高い。'},
  {rank:2,id:'advantest',name:'アドバンテスト',ticker:'6857.T',country:'JP',currRef:'¥27,300前後',tgtRef:'¥35,000',multi:1.3,horizon:'18ヶ月',moat:'AI GPUテスタ事実上独占',catalysts:['Rubin世代でテスト時間3〜4倍化','HBM4テスト需要加速','生産能力5,000→10,000台倍増'],rationale:'NVIDIAのBlackwell→Rubinへの移行でテスト難易度が非線形に増加。H100→B200でテスト時間2〜3倍、Rubin→HBM4でさらに倍増が見込まれる。FY25Q1営業利益率47%という異常な収益性はそのまま拡張される構造。Installed Baseのストック収益も積み上がり中。',risk:'NVIDIA依存度70%超でAIバブル崩壊時のリスク高。高バリュエーション継続中。'},
  {rank:3,id:'lasertec',name:'レーザーテック',ticker:'6920.T',country:'JP',currRef:'¥39,100前後',tgtRef:'¥58,000',multi:1.5,horizon:'18ヶ月',moat:'EUVマスク検査装置100%独占',catalysts:['High-NA EUV普及で装置単価上昇','Rapidus千歳ファブ稼働','Samsung 2nm GAA量産開始'],rationale:'ASML EUVを1台導入するたびにレーザーテック装置も1台必要という連動構造は不変。High-NA EUV（NA=0.55）移行でマスクの複雑性が急増し、検査精度要求と単価が上昇。Rapidus国内顧客追加でリスク分散も進む。',risk:'ピーク時に比べバリュエーション調整は進行済みだが、TSMC設備投資サイクルに依存。'},
  {rank:4,id:'broadcom',name:'Broadcom',ticker:'AVGO',country:'US',currRef:'$382前後',tgtRef:'$650',multi:1.7,horizon:'18ヶ月',moat:'ハイパースケーラXPU設計独占',catalysts:['推論AI需要でカスタムXPU急成長','OpenAI $10B ASIC契約','Google/Meta/Apple全顧客化'],rationale:'AI推論フェーズ移行でカスタムASIC（XPU）の経済合理性が高まりNVIDIA依存分散が加速。Google・Meta・Apple・OpenAIという最大クラウド4社が全てBroadcom顧客。NVIDIA独占に対するヘッジとして大規模発注継続。時価総額$1Tから$2Tへの道筋が見える。',risk:'TSMCの先端製造容量確保が課題。VMwareソフト事業との統合消化が続く。'},
  {rank:5,id:'skhynix',name:'SK hynix',ticker:'000660.KS',country:'KR',currRef:'₩2,150,000前後',tgtRef:'₩3,010,000',multi:1.4,horizon:'18ヶ月',moat:'HBM世界シェア62%（断トツ1位）',catalysts:['HBM4（Rubin向け）独占的供給','AIメモリ・スーパーサイクル継続','Cheongju新工場$12.9B稼働（2028）'],rationale:'HBM3E/HBM4でNVIDIA向け供給を主導し、2026年に時価総額$1兆ドルを突破、世界で最も収益性の高いメモリ企業に。AIメモリ・スーパーサイクルでHBM供給能力の希少性がプレミアムを正当化。ただし2026年初来で大きく上昇しており、ここからの倍率は相対的に低下している。',risk:'NVIDIA依存度が高く、AI需要が反転した際の在庫・価格リスク。Samsung・Micronの追走。'},
  {rank:6,id:'arm',name:'ARM Holdings',ticker:'ARM',country:'UK',currRef:'$377前後',tgtRef:'$565',multi:1.5,horizon:'18ヶ月',moat:'モバイルSoC 99%採用・データセンターCPU侵食',catalysts:['データセンターCPUでx86シェア25%超へ','ロイヤリティ単価上昇（高価値チップ増）','Qualcomm訴訟完全勝訴で事業リスク消滅'],rationale:'AWS Graviton4・Google Axion2・Microsoft Cobalt2など大手クラウドがARMサーバーCPUを積極採用しx86依存度を引き下げ中。ARMのロイヤリティ収入はチップ価格に連動するため、高価値AI SoCの増加で平均単価（ARP）が上昇する構造。',risk:'RISC-Vの中国政策採用と国産化圧力。成長期待の高バリュエーション。'},
  {rank:7,id:'nvidia',name:'NVIDIA',ticker:'NVDA',country:'US',currRef:'$205前後',tgtRef:'$308',multi:1.5,horizon:'18ヶ月',moat:'AIアクセラレータ80〜90%独占・CUDAの壁',catalysts:['Vera Rubin（2026後半）立ち上げ','データセンター支出$500B超へ','CUDAエコシステムの拡張'],rationale:'$5兆ドル時価総額・世界首位に到達後も、Vera Rubin世代でGPU性能が再び大幅向上。CUDAのスイッチングコストはむしろ高まり離脱が困難な状況が続く。FY2026データセンター売上$193.7B（過去最高）からさらなる拡大が見込まれる。',risk:'カスタムASIC（Broadcom/Marvell）が推論用途でシェア侵食。中国輸出規制の影響継続。'},
  {rank:8,id:'kioxia',name:'キオクシア',ticker:'285A.T',country:'JP',currRef:'¥81,200前後',tgtRef:'¥122,000',multi:1.5,horizon:'18ヶ月',moat:'NAND世界3位・BiCS FLASH技術',catalysts:['AIメモリ・スーパーサイクルでNAND急騰','2026年の生産能力は実質完売','AI向けeSSD需要の構造的拡大'],rationale:'2024年12月上場後、AIメモリ・スーパーサイクルでNAND価格が歴史的に高騰し、株価は上場来約13倍に。2026年の生産能力は実質完売で、AI向けストレージ需要が構造的に拡大している。',risk:'NAND価格サイクルの反転リスク。急騰後の高バリュエーション。'},
  {rank:9,id:'tsmc',name:'TSMC',ticker:'TSM',country:'TW',currRef:'$424前後（ADR）',tgtRef:'$595',multi:1.4,horizon:'18ヶ月',moat:'先端ロジック≤3nm 90%超独占',catalysts:['N2量産2025年Q4始動→2026加速','Arizona複数ファブ稼働','CoWoS生産能力拡大継続'],rationale:'AI半導体需要の最終受益者として最も確実な成長軌道上にある。N2量産・CoWoS拡張・地理的分散の三本柱が揃い、台湾地政学リスクのディスカウントが縮小傾向。NVIDIA最大顧客化でトップライン成長が加速。',risk:'台湾有事リスクは依然として株価のキャップ要因。CapEx負担の継続。'},
  {rank:10,id:'disco',name:'ディスコ',ticker:'6146.T',country:'JP',currRef:'¥79,900前後',tgtRef:'¥112,000',multi:1.4,horizon:'18ヶ月',moat:'ダイシング装置世界70%独占',catalysts:['HBM積層でTSV工程需要急増','CoWoS対応ダイシング好調','AIサーバー1台あたりGPU数増加'],rationale:'HBMの3D積層（HBM4は16層以上）で1チップあたりのダイシング・グラインディング工程数が増加。AIサーバー1台に搭載されるHBMの量が増えることで装置需要が非線形に拡大。',risk:'HBM需要の変動感応。高バリュエーション時期もある。'},
  {rank:11,id:'asml',name:'ASML',ticker:'ASML',country:'NL',currRef:'$1,860前後',tgtRef:'$2,600',multi:1.4,horizon:'18ヶ月',moat:'EUV露光装置100%独占',catalysts:['High-NA EUV（NXE:5000）普及加速','中国規制の株価影響ほぼ織り込み済み','2030年まで受注残積み上がり'],rationale:'中国向けDUV輸出規制による売上影響はほぼ株価に織り込まれた状況。High-NA EUVの量産採用（TSMC、Samsung）が進み1台800億円超の装置が出荷される。TSMCの世界3拠点展開で需要の地理的分散も実現。',risk:'中国規制の追加強化リスク。Nikon ArF新世代機による部分的競合（2028年以降）。'},
  {rank:12,id:'amat',name:'Applied Materials',ticker:'AMAT',country:'US',currRef:'$571前後',tgtRef:'$800',multi:1.4,horizon:'18ヶ月',moat:'WFE装置売上世界1位・CMP60%独占',catalysts:['ハイブリッドボンディング（HBM先進パッケージ）参入','Gate-All-Around移行で成膜装置需要増','AI需要でWFE市場$120B超へ'],rationale:'装置売上世界1位としてWFE市場全体の拡大から恩恵。BESIへの出資でハイブリッドボンディングという高成長新市場にも布石。GAA（Gate-All-Around）移行でAMATの得意とする精密成膜・エッチング需要が増加。',risk:'中国向け規制で一部装置の輸出制限。装置サイクルの影響。'},
  {rank:13,id:'micron',name:'Micron Technology',ticker:'MU',country:'US',currRef:'$990前後',tgtRef:'$1,386',multi:1.4,horizon:'18ヶ月',moat:'HBM急拡大（21%・Samsung超え）',catalysts:['HBM3E/HBM4をBlackwell/Rubin向け採用','広島ファブ稼働でHBM生産能力拡大','メモリ・スーパーサイクルで価格急騰'],rationale:'HBM市場でSamsungを追い抜き2位に浮上。FY2025は過去最高売上$37.4Bを記録し、メモリ・スーパーサイクルで売上・粗利率とも過去最高を更新中。広島工場が日本政府補助付きで稼働し固定費を抑制。',risk:'Samsungが品質問題を解決した場合の競争激化。メモリサイクルの反転リスク。'},
  {rank:14,id:'shinetsu',name:'信越化学工業',ticker:'4063.T',country:'JP',currRef:'¥6,858前後',tgtRef:'¥9,250',multi:1.35,horizon:'18ヶ月',moat:'Siウェーハ世界1位（30%）・EUVレジスト',catalysts:['Rapidus/JASM稼働でウェーハ需要増','EUVフォトレジスト展開拡大','PVC・半導体材料の多重独占'],rationale:'ウェーハとEUVレジストの両方を供給できる唯一の企業として構造的に安定。Rapidus・JASM（TSMC熊本）・Micron広島の稼働でウェーハ需要が拡大。高い安定性と配当魅力を持つ防衛的優良株。',risk:'ウェーハサイクルでのASP下落リスク。南海トラフ地震への工場集中リスク。'},
  {rank:15,id:'lam',name:'Lam Research',ticker:'LRCX',country:'US',currRef:'$367前後',tgtRef:'$514',multi:1.4,horizon:'18ヶ月',moat:'エッチング装置世界45%独占',catalysts:['HBM TSVエッチング需要増','3D NAND積層数増加（200層超へ）','GAA移行でエッチング工程数増加'],rationale:'HBM3→HBM4移行で積層枚数が増えTSV（シリコン貫通電極）工程のエッチング需要が増加。3D NANDの更なる高積層化（200〜300層）も同社装置が必須。WFE市場拡大と相まって安定した成長基盤。',risk:'中国向け規制で一部装置の輸出制限。装置サイクル感応。'},
];
